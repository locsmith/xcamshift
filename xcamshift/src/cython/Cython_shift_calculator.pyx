...
cdef class CDSSharedVectorFloat:
    ...
    def __iadd__(self,CDSSharedVectorFloat other):
        cdef int offset
        for i in range(self.size):
            for j in range(self._ensemble_size):
                offset = self.array_offset(i,j)
                addToSharedVectorValue(self.data,offset,getSharedVectorValue(other.data,offset))
        return self
