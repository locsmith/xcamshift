...
from  xplor_access cimport norm,Vec3, Dihedral, Atom,  dot,  cross,  Simulation, CDSVector, DerivList, clearVector, EnsembleSimulation
from shared_vector cimport createSharedVec 
...
cdef class Fast_energy_calculator:
    ...
    cdef Constant_cache* _get_energy_terms(Fast_energy_calculator self, int target_atom_index) nogil:
        return &self._energy_term_cache[target_atom_index]
    
    cdef inline float  _get_calculated_atom_shift(Fast_energy_calculator self, int index) nogil:
        return self._theory_shifts[0][index]
    
    cdef inline float _get_observed_atom_shift(Fast_energy_calculator self, int index) nogil:
