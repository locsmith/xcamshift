...
class TestUnittest(unittest2.TestCase):
    ...
    def test_new_runner_old_case(self):
        runner = unittest2.TextTestRunner(resultclass=resultFactory,
                                          stream=StringIO())
        class TestObservedShiftTable(unittest.TestCase):
            def testOne(self):
                pass
        suite = unittest2.TestSuite((TestObservedShiftTable('testOne'),))
    ...
    def test_old_runner_new_case(self):
        ...
        suite = unittest.TestSuite((TestObservedShiftTable('testOne'),))
        result = runner.run(suite)
        self.assertEqual(result.testsRun, 1)
        self.assertEqual(len(result.errors), 0)
